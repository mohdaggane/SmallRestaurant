...
if (is_post()) {
    ...
    if ($action === 'delete') {
        ...
        if ($id === TRIAL_PLAN_ID) {
            flash(__('pl.err_trial'), 'danger');
        } elseif ($inUse > 0) {
            flash(__('pl.err_in_use'), 'danger');
        } else {
...
<div class="grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-4">
    <div class="card">
        <div class="card-header"><?= e($editing ? __('pl.edit') : __('pl.new')) ?></div>
        <div class="card-body">
            <form method="post" class="space-y-3">
                ...
                <div>
                    <label class="label" for="name"><?= e(__('lbl.name')) ?></label>
                    <input type="text" id="name" name="name" class="input" maxlength="50" required value="<?= e($editing['name'] ?? '') ?>">
                </div>
                <div>
                    <label class="label" for="price_month"><?= e(__('pl.price_month')) ?></label>
                    <input type="number" id="price_month" name="price_month" class="input" step="0.01" min="0" value="<?= e($editing['price_month'] ?? '0.00') ?>">
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="label" for="max_users"><?= e(__('pl.max_users')) ?></label>
                        <input type="number" id="max_users" name="max_users" class="input" min="1" value="<?= e($editing['max_users'] ?? '') ?>" placeholder="<?= e(__('lbl.unlimited')) ?>">
                    </div>
                    <div>
                        <label class="label" for="max_menu_items"><?= e(__('pl.max_items')) ?></label>
                        <input type="number" id="max_menu_items" name="max_menu_items" class="input" min="1" value="<?= e($editing['max_menu_items'] ?? '') ?>" placeholder="<?= e(__('lbl.unlimited')) ?>">
                    </div>
                </div>
                <div>
                    <label class="label" for="sort_order"><?= e(__('lbl.sort_order')) ?></label>
                ...
                <label class="flex items-center gap-2 text-sm">
                    ...
                    <?= e(__('pl.offered_help')) ?>
                </label>
                <div class="flex gap-2">
                    <button class="btn btn-brand"><?= e($editing ? __('pl.save') : __('pl.add')) ?></button>
                    <?php if ($editing): ?>
                        <a class="btn btn-outline" href="<?= url('platform/plans.php') ?>"><?= e(__('btn.cancel')) ?></a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
    ...
    <div class="card">
        <table class="tbl">
            <thead><tr>
                <th><?= e(__('pf.plan')) ?></th><th class="text-right"><?= e(__('pl.price_col')) ?></th><th class="text-right"><?= e(__('nav.users')) ?></th>
                <th class="text-right"><?= e(__('nav.menu_items')) ?></th><th class="text-right"><?= e(__('pf.restaurants')) ?></th><th class="text-center"><?= e(__('pl.offered')) ?></th><th></th>
            </tr></thead>
            <tbody>
            <?php foreach ($plans as $p): ?>
                <tr>
                    <td class="font-semibold"><?= e($p['name']) ?><?= (int)$p['id'] === TRIAL_PLAN_ID ? ' <span class="badge badge-warning">' . e(__('pl.signup_badge')) . '</span>' : '' ?></td>
                    ...
                    <td class="text-right text-nowrap">
                        <a class="btn btn-outline btn-sm" href="?edit=<?= (int)$p['id'] ?>"><?= e(__('btn.edit')) ?></a>
                        <form method="post" class="inline" onsubmit="return confirm(<?= e(json_encode(__('pl.delete_confirm'))) ?>)">
                            <?= csrf_field() ?>
                            <input type="hidden" name="action" value="delete">
                            <input type="hidden" name="id" value="<?= (int)$p['id'] ?>">
                            <button class="btn btn-outline btn-sm text-bad"><?= e(__('btn.delete')) ?></button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
